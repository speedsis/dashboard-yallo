...
# Algumas limpezas
df['atendimento'] = df['atendimento'].str.lstrip('R$ ')
...
df['atendimento'] = df['atendimento'].astype(int)
...
def graph1(month, toggle):
    ...
    df_1 = df_1.groupby(['Equipe', 'Consultor'])['atendimento'].sum()
    df_1 = df_1.sort_values(ascending=False)
    df_1 = df_1.groupby('Equipe').head(1).reset_index()

    fig2 = go.Figure(go.Pie(labels=df_1['Consultor'] + ' - ' + df_1['Equipe'], values=df_1['atendimento'], hole=.6))
    fig1 = go.Figure(go.Bar(x=df_1['Consultor'], y=df_1['atendimento'], textposition='auto', text=df_1['Atendimento']))
...
def graph5(month, toggle):
    ...
    df_5 = df_5.groupby(['Consultor', 'Equipe'])['atendimento'].sum()
    df_5.sort_values(ascending=False, inplace=True)
    df_5 = df_5.reset_index()
    fig5 = go.Figure()
    fig5.add_trace(go.Indicator(mode='number',
        title = {"text": f"<span>{df_5['Consultor'].iloc[0]} - Top Produtivo</span><br><span style='font-size:70%'>Produtividade - em relação a média</span><br>"},
 

        value =  df_5['atendimento'].iloc[0],
        
        delta = {'relative': True, 'valueformat': '.1%', 'reference': df_5['atendimento'].mean()}
    ))
    ...
    df_6 = df_6.groupby('Equipe')['atendimento'].sum()
...
def graph7(toggle):
    ...
    df_7 = df.groupby(['Mês', 'Equipe'])['atendimento'].sum().reset_index()
    df_7_group = df.groupby('Mês')['atendimento'].sum().reset_index()
    
    fig7 = px.line(df_7, y="atendimento", x="Mês", color="Equipe")
    fig7.add_trace(go.Scatter(y=df_7_group["atendimento"], x=df_7_group["Mês"], mode='lines+markers', fill='tonexty', name='Total de Vendas'))
...
def graph8(month, toggle):
    ...
    df_8 = df_8.groupby('Equipe')['atendimento'].sum().reset_index()
    fig8 = go.Figure(go.Bar(
        x=df_8['atendimento'],
        y=df_8['Equipe'],
        orientation='h',
        textposition='auto',
        text=df_8['atendimento'],
...
def graph9(month, team, toggle):
    ...
    df_9 = df_9.groupby('Meio de Propaganda')['atendimento'].sum().reset_index()

    fig9 = go.Figure()
    fig9.add_trace(go.Pie(labels=df_9['Meio de Propaganda'], values=df_9['atendimento'], hole=.7))
...
def graph10(team, toggle):
    ...
    df10 = df_10.groupby(['Meio de Propaganda', 'Mês'])['atendimento'].sum().reset_index()
    fig10 = px.line(df10, y="atendimento", x="Mês", color="Meio de Propaganda")
...
def graph11(month, team, toggle):
    ...
    fig11.add_trace(go.Indicator(mode='number',
        title = {"text": f"<span style='font-size:150%'>Atendimentos/mês</span><br>"},
        value = df_11['atendimento'].sum(),
        number = {'prefix': ""}
    ))
